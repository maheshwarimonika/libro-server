...
router.post('/', (req,res) => {
    Book.create({ ...req.body.book, req.currentUser._id })
...
router.get('/search',(req,res)=>{
  request
  .get(
    `https://www.goodreads.com/search/index.xml?key=JNR0Gyqk4C6i6KK24h1Yg&q=${req.query.q}`
  )
  .then(result =>
...
router.get('/fetchPages',(req,res)=>{
  ...
  request
  .get(
    `https://www.goodreads.com/book/show.xml?key=JNR0Gyqk4C6i6KK24h1Yg&id=${goodreadsId}`
  )
  .then(result =>
        parseString( result, (err, goodreadsResult) => {
